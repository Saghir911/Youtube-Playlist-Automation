...
async function customComment(aiComment: string): Promise<void> {
  if (state.commentDone) return console.log("Comment already added—skipping.");
  try {
    ...
    if (!inputValue) return console.warn("Comment input value field not found");
...
// AI Comment Fetcher
async function getCommentText(videoUrl: string, prompt: string): Promise<void> {
  try {
    ...
    const commentFromAi = (data?.choices?.[0]?.message?.content || "").replace(
      /^"(.*)"$/,
      "$1"
    );
